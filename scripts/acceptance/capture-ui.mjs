...
async function main() {
  ...
  try {
    ...
    for (const screen of requiredScreens) {
      const captures = [];
      const desktopViewport = parseViewport(screen.viewport);
      const screenViewports = desktopViewport
        ? viewports.map((viewport) => (viewport.name === "desktop" ? { ...viewport, ...desktopViewport } : viewport))
        : viewports;
      for (const viewport of screenViewports) {
